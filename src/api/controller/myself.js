...
module.exports = class extends Base {
    ...
    async getMyActivityListAction() {
        ...
        if (think.isEmpty(hasjoin)) {
            data = await model.query("select sa.studentID,sa.activityID,sa.shstate,sa.createdate, a.activityName,a.sponsor,a.startDate, a.endDate,case when (a.endDate > now() and now() > a.startDate) then '进行中' when a.endDate < now() then '已完成' when a.startDate > now() then '已报名' end as hasjoin from culture_student_activity sa left join culture_activity a on sa.activityid=a.activityid where sa.studentid="+id+" limit "+start+","+pagesize+"");
            ...
            data = await model.query("select sa.studentID,sa.activityID,sa.shstate,sa.createdate, a.activityName,a.sponsor,a.startDate, a.endDate,case when (a.endDate > now() and now() > a.startDate) then '进行中' when a.endDate < now() then '已完成' when a.startDate > now() then '已报名' end as hasjoin from culture_student_activity sa left join culture_activity a on sa.activityid=a.activityid where sa.studentid="+id+" and (a.endDate > now() and now() > a.startDate) limit "+start+","+pagesize+"");
            ...
            data = await model.query("select sa.studentID,sa.activityID,sa.shstate,sa.createdate, a.activityName,a.sponsor,a.startDate, a.endDate,case when (a.endDate > now() and now() > a.startDate) then '进行中' when a.endDate < now() then '已完成' when a.startDate > now() then '已报名' end as hasjoin from culture_student_activity sa left join culture_activity a on sa.activityid=a.activityid where sa.studentid="+id+" and a.endDate < now() limit "+start+","+pagesize+"");
            ...
            data = await model.query("select sa.studentID,sa.activityID,sa.shstate,sa.createdate, a.activityName,a.sponsor,a.startDate, a.endDate,case when (a.endDate > now() and now() > a.startDate) then '进行中' when a.endDate < now() then '已完成' when a.startDate > now() then '已报名' end as hasjoin from culture_student_activity sa left join culture_activity a on sa.activityid=a.activityid where sa.studentid="+id+" and a.startDate > now() limit "+start+","+pagesize+"");
